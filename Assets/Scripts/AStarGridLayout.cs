    ...
    void Awake()
    {
        ...
        for (int i = 0; i < GridSize; ++i)
        {
            for (int j = 0; j < GridSize; ++j)
            {
                ...
                if (Physics.Raycast(new Vector3(xPos, _terrainPosition.y + _terrainSize.y, zPos),
        ...
        for (int i = 0; i < GridSize; ++i)
        {
            for (int j = 0; j < GridSize; ++j)
            {
                ...
                if (i - 1 >= 0)
                {
                    ...
                    Gizmos.DrawLine(_grid[i, j].PositionInWorld + Vector3.up * 0.05f,
                        _grid[i - 1, j].PositionInWorld + Vector3.up * 0.05f);
                }
                ...
                if (j - 1 >= 0)
                {
                    ...
                    Gizmos.DrawLine(_grid[i, j].PositionInWorld + Vector3.up * 0.05f,
                        _grid[i, j - 1].PositionInWorld + Vector3.up * 0.05f);
                }
                ...
                if (i - 1 >= 0 && j - 1 >= 0)
                {
                    ...
                    Gizmos.DrawLine(_grid[i, j].PositionInWorld + Vector3.up * 0.05f,
                        _grid[i - 1, j - 1].PositionInWorld + Vector3.up * 0.05f);
                }
            }
        }
    }
